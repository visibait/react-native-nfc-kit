...
<Warning>
  **Chrome on Android, and nothing else** at the time of writing. Not Safari, not Firefox, not
  desktop Chrome. `nfc.isSupported()` answers honestly, so a page can hide its NFC affordance rather
...
| `tag.makeReadOnly()`           | Works where the browser implements it      |
...
<Note>
  `getNdefStatus` rejecting is deliberate. Web NFC reports neither whether a tag is writable nor how
  much room it has, and returning invented numbers would only move the failure to the write, where
  it would surface as something else entirely.
</Note>
...
<AccordionGroup>
  <Accordion title="Writing a smart poster" icon="image">
  ...
  <Accordion
    title="A record type Web NFC reports that this library does not recognise"
...
one platform, and any app that needs a tag on iOS needs a native build anyway.

<Tip>
  What it is genuinely good for is a debugging page, an internal tool, or reading a tag from a
  laptop-adjacent workflow — and [the NDEF codec](/ndef) runs anywhere with no NFC at all, including
  Node, if all you need is to build or parse a message.
</Tip>
