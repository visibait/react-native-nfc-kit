...
<CardGroup cols={2}>
  <Card title="Before you trust a combination" icon="clipboard-check">
    Run the rows that apply to the device, chipset and tag type you are shipping on.
  </Card>
  <Card title="When something misbehaves" icon="bug">
...
<Note>
  The columns start empty on purpose. Rather than carry one person's results as though they were
...
<Warning>
  **Passed** when the thirtieth iteration behaves like the first, and step 5 still works. **Failed**
  if anything starts reporting `systemBusy`, if scanning stops starting, or if latency grows across
  iterations — all three are the signature of a session or listener that was not released.
</Warning>
